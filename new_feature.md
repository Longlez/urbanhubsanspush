...
Nom du microservice : `ms6-validateur-capteur`
...
```text
Capteur IoT simule -> ms-collecte-iot -> ms6-validateur-capteur -> ms-analyse
```

## Taches demandees

- [x] Creer le microservice `ms6-validateur-capteur`.
...
- [x] Ajouter `ms6-validateur-capteur` au `docker-compose.yml` general.
- [x] Rediger la documentation complete du microservice.

## Fonctionnalite exposee

`ms6-validateur-capteur` expose un endpoint `POST /validate`.
...
Les seuils sont configures dans `ms6-validateur-capteur/src/config/sensor_thresholds.py`.
...
|---|---|---|---|
| Workflow CI/CD | OK | `.github/workflows/ms6-validateur-capteur.yml` | 5 jobs sequentiels : `test`, `quality`, `build`, `verify-image`, `deploy-staging`. |
| Code principal | OK | `ms6-validateur-capteur/src/validator.py` | App FastAPI, `Sensordata`, seuils importes, lifespan RabbitMQ. |
| Routes API | OK | `ms6-validateur-capteur/src/adapters/api/routes.py` | `GET /health`, `POST /validate`, `POST /traffic/validate`. |
| Tests | OK | `ms6-validateur-capteur/tests/test_validator.py`, `ms6-validateur-capteur/tests/test_validation.py` | 19 tests au total, dont cas normal/moderate/critical/unknow. |
| Dockerfile | OK | `ms6-validateur-capteur/Dockerfile` | Base `python:3.11-slim`, `EXPOSE 8000`, `CMD uvicorn`. |
| Dependances runtime | OK | `ms6-validateur-capteur/requirements.txt` | Versions fixees pour FastAPI, Uvicorn, Pydantic, Pika, HTTPX. |
| Dependances dev | OK | `ms6-validateur-capteur/requirements-dev.txt` | Pytest, pytest-cov, coverage et flake8 versions. |
| Sonar | OK | `ms6-validateur-capteur/sonar-project.properties` | Coverage et JUnit branches vers `03_rapport_tests`. |

### L2 - Rapports pytest et coverage

| Livrable | Statut | Chemin verifie |
|---|---|---|
| Sortie console pytest | OK | `ms6-validateur-capteur/03_rapport_tests/rapport_tests.txt` |
| Rapport JUnit XML | OK | `ms6-validateur-capteur/03_rapport_tests/rapport_tests.xml` |
| Alias demande `rapport_test.xml` | OK | `ms6-validateur-capteur/03_rapport_tests/rapport_test.xml` |
| Rapport coverage XML | OK | `ms6-validateur-capteur/03_rapport_tests/coverage.xml` |

### L3 - Qualite et securite avant/apres

| Livrable | Statut | Chemin verifie | Notes |
|---|---|---|---|
| Flake8 avant | OK | `ms6-validateur-capteur/reports/quality/flake8_avant.txt` | Fichier present. |
| Flake8 apres | OK | `ms6-validateur-capteur/reports/quality/flake8_apres.txt` | Fichier present. |
| Sonar avant | OK avec reserve | `ms6-validateur-capteur/reports/quality/sonar_avant.png` | Placeholder local, le scan reel est distant via SonarCloud. |
| Sonar apres | OK avec reserve | `ms6-validateur-capteur/reports/quality/sonar_apres.png` | Placeholder local, le scan reel est distant via SonarCloud. |
| Snyk avant | OK avec reserve | `ms6-validateur-capteur/reports/security/snyk_avant.txt` | Trace locale indiquant que le CLI Snyk n'etait pas installe. |
| Snyk apres | OK avec reserve | `ms6-validateur-capteur/reports/security/snyk_apres.txt` | Trace locale indiquant que le CLI Snyk n'etait pas installe. |
| Synthese avant/apres | OK | `ms6-validateur-capteur/reports/synthese_avant_apres.md`, `ms6-validateur-capteur/reports/synthese_avant_apres.pdf` | Synthese dediee au microservice. |

### L4 - Deploiement et integration

| Livrable | Statut | Chemin verifie | Notes |
|---|---|---|---|
| Build image | OK | `.github/workflows/ms6-validateur-capteur.yml` | Image poussee vers `ghcr.io/<org>/ms6-validateur-capteur:<sha>`. |
| Verification conteneur | OK | `.github/workflows/ms6-validateur-capteur.yml` | `docker run` et verification `GET /health` sur le port `8000`. |
| Compose staging | OK | `docker-compose.yml` | Service `ms6-validateur-capteur` expose sur `8006:8000`. |

## Documentation technique Diataxis

La documentation du microservice est disponible dans `ms6-validateur-capteur/docs/` et applique la methode Diataxis au systeme d'information du validateur :
...
- [x] Le nom de dossier reel utilise est `ms6-validateur-capteur`.
- [x] Le workflow surveille `ms6-validateur-capteur/**` et conserve aussi `ms6-validateur-capteur-capteur/**` pour compatibilite avec l'intitule initial.
...
- [x] Les rapports de tests sont generes dans `ms6-validateur-capteur/03_rapport_tests/`.
- [x] La documentation demandee est reorganisee selon Diataxis.
- [x] La taille du dossier `ms6-validateur-capteur` reste inferieure a 5 Mo hors caches Python et artefacts de virtualenv.
...
```bash
docker build -t ms6-validateur-capteur:local ms6-validateur-capteur
